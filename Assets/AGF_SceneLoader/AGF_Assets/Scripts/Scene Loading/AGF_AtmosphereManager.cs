...
public class AGF_AtmosphereManager : MonoBehaviour {
	...
	public class SkyboxProperties : AtmosphereProperties{
		...
		public string underwaterSkyboxID{
			...
			set { if ( value != _underwaterSkyboxID ) { hasChanged = true; _underwaterSkyboxID = value; } }
		}
		...
		public override void UpdateOnMode(){
			...
			if ( hasChanged ){
				if ( _active ){
					...
					print("Skybox Material: " + m_CameraManager.GetMainCamera().GetComponent<Skybox>().material );
					if(m_CameraManager.GetMainCamera().GetComponent<Skybox>().material != null){
						m_CameraManager.GetMainCamera().GetComponent<Skybox>().material.shader = Shader.Find ("AGF/Skybox Cubed Rotate");
						print("Skybox Shader: " + m_CameraManager.GetMainCamera().GetComponent<Skybox>().material.shader );
					}
					else{
						m_CameraManager.GetMainCamera().GetComponent<Skybox>().material = new Material(Shader.Find("AGF/Skybox Cubed Rotate"));
					}
				} else {
		...
		public override void UpdateUnderwaterMode(){
			Skybox m_CameraSkybox = m_CameraManager.GetMainCamera().GetComponent<Skybox>();
			
			if ( hasChanged ){
				string[] split = _underwaterSkyboxID.Split( new char[]{'/'} );
				print( _underwaterSkyboxID + " " + split[0] + " " + split[1] );
				m_CameraSkybox.material = (Material)m_SaveDataManager.LoadAssetFromBundle( split[0], split[1] + ".mat" );
				
				Quaternion rot = Quaternion.Euler (0f, _rotation, 0f);
				Matrix4x4 m = Matrix4x4.TRS (Vector3.zero, rot, new Vector3(1,1,1) );
				if(m_CameraSkybox.material){
					m_CameraSkybox.material.SetMatrix("_Rotation", m);
				}
				else{
					m_CameraSkybox.material = null;
					Resources.UnloadUnusedAssets();
				}
			}
			
			m_CameraSkybox.material.SetColor("_Tint", _underwaterTint );
			if ( _underwaterAutoRotate ) _underwaterRotation += _underwaterAutoRotationSpeed;
	...
	private bool m_UnderwaterFogVisible = false;
	...
	private void Update(){
		if(m_AtmosphereProperties == null)
		m_AtmosphereProperties = new AtmosphereProperties[]{ m_CurrentSkybox, m_CurrentHaze, m_CurrentBasicFog, m_CurrentFillLight, m_CurrentBackLight, m_CurrentKeyLight, m_CurrentAmbientLight };
		
		// update the global light rotation seperately.
		m_CurrentLightKit.rotation = Quaternion.Euler( 0.0f, m_GlobalLightRotation, 0.0f );
		
		Transform waterBase = null;
		if(FindObjectOfType<WaterBase>()){
			waterBase = FindObjectOfType<WaterBase>().transform;
		}
		if ( m_AtmosphereVisible == false || m_CurrentSubState == SubState.Off ){
			...
			for ( int i = 0; i < m_AtmosphereProperties.Length; i++ ){
				if(m_AtmosphereProperties[i] == null)
				m_AtmosphereProperties = new AtmosphereProperties[]{ m_CurrentSkybox, m_CurrentHaze, m_CurrentBasicFog, m_CurrentFillLight, m_CurrentBackLight, m_CurrentKeyLight, m_CurrentAmbientLight };
				else
					m_AtmosphereProperties[i].UpdateOnMode();
			}
//			if(waterBase  && waterBase.position.y > Camera.main.transform.position.y)
//				ChangeState(SubState.Underwater);
		} else if ( m_CurrentSubState == SubState.Underwater ){
			for ( int i = 0; i < m_AtmosphereProperties.Length; i++ ){
				m_AtmosphereProperties[i].UpdateUnderwaterMode();	
			}
			if(waterBase && waterBase.position.y < Camera.main.transform.position.y)
				ChangeState(SubState.On);
		}
	}
	...
	public void SetLightFromString( string serializationString ){
		...
		if(split.Length >= i + 14){
			// ambient light
			int.TryParse( split[++i], out outInt );
			
			// ambient color
			float r2,g2,b2,a2;
			float.TryParse( split[++i], out r2 ); 
			float.TryParse( split[++i], out g2 ); 
			float.TryParse( split[++i], out b2 ); 
			float.TryParse( split[++i], out a2 );
			...
			// key color
			float r3,g3,b3,a3;
			float.TryParse( split[++i], out r3 ); 
			float.TryParse( split[++i], out g3 ); 
			float.TryParse( split[++i], out b3 ); 
			float.TryParse( split[++i], out a3 );
			
			m_CurrentAmbientLight.underwaterTint = new Color(r2,g2,b2,a2);
			m_CurrentKeyLight.underwaterTint = new Color(r3,g3,b3,a3);
		}
		else{
			m_CurrentAmbientLight.underwaterTint = new Color(r,g,b,a);
			m_CurrentKeyLight.underwaterTint = new Color(r,g,b,a);
		}
		
		m_CurrentAmbientLight.underwaterActive = (outInt == 1);
		m_CurrentKeyLight.underwaterYawAngle = yawAngle;
		m_CurrentKeyLight.underwaterPitchAngle = pitchAngle;
		m_CurrentKeyLight.underwaterIntensity = intensity;
	...
	public void SetSkyboxFromString( string serializationString ){
		...
		float r1,g1,b1,a1;
		float.TryParse( split[++i], out r1 );
		float.TryParse( split[++i], out g1 );
		float.TryParse( split[++i], out b1 );
		float.TryParse( split[++i], out a1 );
		m_CurrentSkybox.tint = new Color(r1,g1,b1,a1);
		...
		float.TryParse( split[++i], out b2 );
		float.TryParse( split[++i], out a2 );
		m_CurrentSkybox.clearColor = new Color(r2,g2,b2,a2);
		
		//Set water values if they are saved
		if(split.Length >= i + 13){
			float.TryParse( split[++i], out rotation );
			int.TryParse( split[++i], out autoRotate );
			float.TryParse( split[++i], out autoRotationSpeed );
			...
			m_CurrentSkybox.underwaterSkyboxID = split[++i];
		}
		
		m_CurrentSkybox.underwaterRotation = rotation;
		m_CurrentSkybox.underwaterAutoRotate = (autoRotate == 1 );
		m_CurrentSkybox.underwaterAutoRotationSpeed = autoRotationSpeed;
		
		m_CurrentSkybox.underwaterTint = new Color(r1,g1,b1,a1);
		m_CurrentSkybox.underwaterClearColor = new Color(r2,g2,b2,a2);
		
		if(m_CurrentSkybox.underwaterSkyboxID == null)
			m_CurrentSkybox.underwaterSkyboxID = m_CurrentSkybox.skyboxID;
	...
	public void SetFogFromString( string serializationString ){
		...
		// get all water fog values.
		if(split.Length >= i+21){
			m_CurrentHaze.underwaterActive = ( split[++i] == "1" );
			m_CurrentBasicFog.underwaterActive = ( split[++i] == "1" );
			m_UnderwaterFogVisible = ( split[++i] == "1" );
			...
			float.TryParse( split[++i], out b );
			float.TryParse( split[++i], out a );
			basicFogColor = new Color(r,g,b,a);
			
		}
		
		m_CurrentHaze.underwaterDistanceMode = ( fogModeInt == 1 );
		m_CurrentHaze.underwaterTint = fogColor;
		m_CurrentHaze.underwaterDensity = fogDensity;
		m_CurrentHaze.underwaterStartDist = startDistance;
		m_CurrentHaze.underwaterFalloff = heightScale;
		m_CurrentHaze.underwaterHeight = height;
		
		m_CurrentBasicFog.underwaterLinearMode = ( basicFogModeInt == 1 );
		m_CurrentBasicFog.underwaterTint = basicFogColor;
		m_CurrentBasicFog.underwaterDensity = basicFogDensity;
		m_CurrentBasicFog.underwaterStartDist = basicFogStart;
		m_CurrentBasicFog.underwaterEndDist = basicFogEnd;
