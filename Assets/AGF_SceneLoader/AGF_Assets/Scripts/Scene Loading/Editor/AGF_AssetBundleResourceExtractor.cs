...
//		AssetDatabase.ImportAsset( sourceFolder + "/" + sceneFolder + "/Skyboxes/skyboxMaterial.mat" );
	...
	public static void ExtractTerrainTextures(){
		...
		if ( template == null ){
			Debug.LogWarning ("Template was null :(");			
		} else {
...
//					AssetDatabase.ImportAsset( sourceFolder + "/" + sceneFolder + "/Warehouse/" + materialPath );
		...
		if(GameObject.Find(sceneFolder))
			tile.transform.parent = GameObject.Find (sceneFolder).transform;
	...
	public static void ExtractVoxelObject( Transform tile ){
		...
		// part B: MESH FILTERS
		Voxel.Chunk[] objectMeshFilters = tile.GetComponentsInChildren<Voxel.Chunk>();
		
		for ( int i = 0; i < objectMeshFilters.Length; i++ ){
			Mesh sharedMesh = objectMeshFilters[i].hiFilter.sharedMesh;
			if(sharedMesh==null)
				break;
			string meshPath = "Voxels/VoxelTerrain.asset";
			...
			if ( savedMeshList.ContainsKey( sharedMesh ) == false ){
				...
				while ( File.Exists( targetDirectory + "/" + meshPath ) ){
					// a different version of the mesh already exists. we'll need to rename this one.
					meshPath = "/Voxels/VoxelTerrain" + count.ToString() + ".asset";	
					count++;
				}
			...
			// save the mesh reference back into the object.
			objectMeshFilters[i].hiFilter.sharedMesh = (Mesh)AssetDatabase.LoadAssetAtPath( sourceFolder + "/" + sceneFolder + "/Voxels/" + savedMeshList[sharedMesh], typeof(Mesh) );
		...
		for ( int i = 0; i < objRenderers.Length; i++ ){
			...
			for ( int j = 0; j < rendererMaterials.Length; j++ ){
				...
				if ( savedMaterialList.ContainsKey( sharedMaterial ) == false ){
					...
					if(!Directory.Exists(sourceFolder + "/" + sceneFolder + "/Voxels/Materials"))
						Directory.CreateDirectory(sourceFolder + "/" + sceneFolder + "/Voxels/Materials");
					if(!Directory.Exists(sourceFolder + "/" + sceneFolder + "/Voxels/Materials/AGF"))
						Directory.CreateDirectory(sourceFolder + "/" + sceneFolder + "/Voxels/Materials/AGF");
					if(!Directory.Exists(sourceFolder + "/" + sceneFolder + "/Voxels/Materials/Legacy Shaders"))
						Directory.CreateDirectory(sourceFolder + "/" + sceneFolder + "/Voxels/Materials/Legacy Shaders");
...
//					AssetDatabase.ImportAsset( sourceFolder + "/" + sceneFolder + "/Warehouse/" + materialPath );
				...
				// save the material reference back into the object.
				Material mat = (Material)AssetDatabase.LoadAssetAtPath( sourceFolder + "/" + sceneFolder + "/Voxels/" + savedMaterialList[sharedMaterial], typeof(Material) );
				rendererMaterials[j] = mat;
				objRenderers[i].sharedMaterials = rendererMaterials;
			}
		}
