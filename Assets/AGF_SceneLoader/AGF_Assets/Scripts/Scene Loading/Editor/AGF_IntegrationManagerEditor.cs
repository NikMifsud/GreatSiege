...
public class AGF_IntegrationManagerEditor : Editor {
	...
	private void AddAllComponents( Camera cam ){
		if (!cam.GetComponent<SSAOEffect> ())
			cam.gameObject.AddComponent<SSAOEffect> ();
		if (!cam.GetComponent<Skybox> ())
			cam.gameObject.AddComponent<Skybox> ();
		if (!cam.GetComponent<GlobalFog> ())
			cam.gameObject.AddComponent<GlobalFog> ();
		if (!cam.GetComponent<BloomAndLensFlares> ())
			cam.gameObject.AddComponent<BloomAndLensFlares> ();
		if (!cam.GetComponent<ContrastEnhance> ())
			cam.gameObject.AddComponent<ContrastEnhance> ();
		if (!cam.GetComponent<AntialiasingAsPostEffect> ())
			cam.gameObject.AddComponent<AntialiasingAsPostEffect> ();
		if (!cam.GetComponent<Vignetting> ())
			cam.gameObject.AddComponent<Vignetting> ();
	}
}
