...
//			currentHeight += 60;
			...
			// Buttons
			if ( GUI.Button ( new Rect( thisRect.width/2.0f - buttonWidth/2.0f, currentHeight, buttonWidth, buttonHeight ), "Select AGF Installation Folder (Steam Users Click Here)" ) ){
...
//			currentHeight += 30.0f;
	...
	private static int LoadScene(){
		...
		if ( System.IO.Directory.Exists( sourceDirectory  + "/Projects") == false ){
			Debug.LogError ("Error: " + sourceDirectory + "/Projects" + " does not exist. If you are a steam user, make sure to specify the source directory that your AGF install lives in. Otherwise, if you have an installer from elsewhere, make sure it has properly been installed.");
			return AGF_ReturnCode.UserDataFolderDoesNotExist;
		}
		
		string filePath = EditorUtility.OpenFilePanel( "Load AGF Scene", sourceDirectory + "/Projects", "agfs" );
