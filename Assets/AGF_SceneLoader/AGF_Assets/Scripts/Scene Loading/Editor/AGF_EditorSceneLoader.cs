...
public class AGF_EditorSceneLoader : EditorWindow {
	...
	private enum SubState{
		LoadingScene, ExtractingWarehouseObjects, ExtractingVoxelObjects, FillingBar,	
	}
	...
	private void OnGUI(){
		...
		} else if ( m_CurrentSubState == SubState.ExtractingVoxelObjects ){
			completionPercent = (float)m_CurrentVoxelObject / (float)chunkList.Length;
			EditorUtility.DisplayProgressBar( "Importing Scene.", "Creating voxels...", completionPercent );
		} else {
		...
		if ( m_CurrentSubState == SubState.LoadingScene ){
			...
			if ( m_FrameDelay > 0 ){
				...
				if ( m_CurrentWarehouseObject >= m_TotalNumberOfWarehouseObjects ){
					if(FindObjectOfType<Voxel.VoxelTerrain>())
						m_CurrentSubState = SubState.ExtractingVoxelObjects;
					else
			...
			if ( m_FrameDelay > 0 ){
				...
				if ( m_CurrentVoxelObject >= 1 ){


					FinishSceneApplication();
				} else {
//					Mesh currentMesh = chunkList[m_CurrentVoxelObject].hiFilter.sharedMesh;
					Voxel.VoxelTerrain voxelTerrain = FindObjectOfType<Voxel.VoxelTerrain>();
					if(voxelTerrain != null){
						foreach(Voxel.Chunk chunk in voxelTerrain.GetComponentsInChildren<Voxel.Chunk>()){
							if(chunk.hiFilter.sharedMesh.vertexCount == 0)
								DestroyImmediate(chunk.gameObject);
							else{
								chunk.hiFilter.transform.parent = voxelTerrain.transform;
								if(chunk.grassFilter)
									chunk.grassFilter.transform.parent = voxelTerrain.transform;
								DestroyImmediate(chunk.gameObject);
							}
						}
						AGF_AssetBundleResourceExtractor.ExtractVoxelObject( voxelTerrain.transform );
					}

					m_CurrentVoxelObject++;
				}
...
//				Debug.Log("special found!");
		...
		if ( m_LoadSceneMode == AGF_LevelLoader.LoadSceneMode.All ){
			...
			chunkList = FindObjectsOfType<Voxel.Chunk>();
			voxelMesh = new Mesh();
		} else if ( m_LoadSceneMode == AGF_LevelLoader.LoadSceneMode.CameraOnly ){
			AGF_AssetBundleResourceExtractor.ExtractSkybox();
