...
namespace Voxel 
{
	public class Chunk : MonoBehaviour 
	{
		...
		public class VoxelVert
		{
			...
			public VoxelVert[] neigs; //max 6 - or no 7 somehow...
			...
			public void Replace ( VoxelVert v2  ) //removing v2, 
			{
			...
			static public void ResetProcessed (System.Collections.Generic.List<VoxelFace> faces)
			{
		...
		{
			public VoxelVert[] verts; //always 9
			public bool  visible;
			...
			public VoxelFace() { verts = new VoxelVert[9]; }
			...
			public VoxelBlock block; //backward link to block. Against ideology.
			public byte dir;
			
			public void Weld ( VoxelFace f2 ,   int p1 ,   int p2  ) //welding two points only
			{
				//no vert - creating it
			...
			public void Weld ( VoxelFace fB ,   int pA2 ,   int pB2 ,   bool inverted  )  //welding three poins (side) using mid-point number
			{
			...
			public void Weld ( VoxelBlock block2 ,   int dir2 ,   int p1 ,   int p2 ,   bool check  )  //same as above, but works only when no p1 (for cross-block)
			{
				if (verts[p1]==null && block2!=null && block2.faces[dir2]!=null && block2.faces[dir2].verts[p2]==null) 
					Weld( block2.faces[dir2], p1, p2, true);
			}
			
			public void Weld ( VoxelBlock block2 ,   int dir2 ,     int pA1 ,   int pA2 ,   int pA3 ,     int pB1 ,   int pB2 ,   int pB3  ) //welding three poins (side) using mid-point number
			{
			...
			{
				if (verts[8].normal.y>0.7f && block.type.shaderSize == Voxel.VoxelBlockType.ShaderSizes.Two){
					return block.type.topTexture;
				}
				else{
					return block.type.texture;
				}
			}
			
			public Texture GetBump ()
			{
				if (verts[8].normal.y>0.7f && block.type.shaderSize == Voxel.VoxelBlockType.ShaderSizes.Two) return block.type.topBumpTexture;
		...
		public class VoxelBlock
		{
			...
			public VoxelFace[] faces; //max 8
			
			public bool  visible;
			public VoxelBlockType type;
			//public float ambient;
			public Chunk chunk;
			
			public VoxelBlock ( int nx ,   int ny ,   int nz  )
			{ 
				x=nx; y=ny; z=nz;
		...
		#region Face holder for collider
		public class FaceHolder //used to boost colliders
		{
		...
		[System.NonSerialized] public FaceHolder[] faceHolders;
		...
		public MeshFilter bottomGrassFilter;
		public MeshFilter constructorFilter;
		public MeshCollider constructorCollider;
	
		[System.NonSerialized] public System.Collections.Generic.List<VoxelFace> visibleFaces = new List<VoxelFace>();
		
		public enum VoxelFaceDir {top=0, bottom=1, front=2, back=3, left=4, right=5}; //y,z,x: left and right are on x axis
		...
		#region function Create, Init, Clear
		static public Chunk CreateChunk (VoxelTerrain land)
		{
		...
		public void Init (int x, int z)
		{
			...
			transform.localPosition = new Vector3(x*(land.chunkSize-land.overlap*2), 0, z*(land.chunkSize-land.overlap*2));
			...
			coordX = x;
			coordZ = z;
			offsetX = x*(land.chunkSize-land.overlap*2); 
			offsetZ = z*(land.chunkSize-land.overlap*2);
			size = land.chunkSize;
		...
		public void Clear ()
		{
			...
			if (grassFilter != null) grassFilter.sharedMesh.Clear();
			if (bottomGrassFilter != null) bottomGrassFilter.sharedMesh.Clear();
			
			terrainProgress = Progress.notCalculated;
			ambientProgress = Progress.notCalculated;
			prefabsProgress = Progress.notCalculated;
			grassProgress = Progress.notCalculated;
			constructorProgress = Progress.notCalculated;
		}
		...
		public void SwitchLod (bool lod)
		{
			...
			//if lod
			if (lod && hiFilter.GetComponent<Renderer>().enabled)
			{
				...
				loFilter.GetComponent<Renderer>().sharedMaterial = hiFilter.GetComponent<Renderer>().sharedMaterial;
			}
			
			//if main
			if (!lod && loFilter.GetComponent<Renderer>().sharedMaterials.Length != 0) //loFilter.renderer.enabled)
			{
				...
				loFilter.GetComponent<Renderer>().sharedMaterials = new Material[0];
			}
		}
		#endregion
		...
		{		
			...
			{
				VoxelVert.ResetProcessed(visibleFaces);
			...
			//Clear() function produces memory leak! Using Destroy instead
			DestroyImmediate(hiFilter.sharedMesh);
			DestroyImmediate(loFilter.sharedMesh);
			
			Mesh hi = hiFilter.sharedMesh;
			if (hi == null) { hi = new Mesh(); hiFilter.sharedMesh = hi; }
			
			Mesh lo = loFilter.sharedMesh;
			if (lo == null) { lo = new Mesh(); loFilter.sharedMesh = lo; }
			#endregion
			...
			Material material = hiFilter.GetComponent<Renderer>().sharedMaterial;
			if (!material) 
			{
			...
			if (loFilter.GetComponent<Renderer>().sharedMaterials.Length != 0) loFilter.GetComponent<Renderer>().sharedMaterial = material;
...
//				if(land.types[t].shaderSize == VoxelBlockType.ShaderSizes.One){
//					material.SetTexture("_TopTex", land.types[t].texture);
//					material.SetTexture("_BottomTex", land.types[t].texture);
//					material.SetTexture("_RightTex", land.types[t].texture);
//					material.SetTexture("_LeftTex", land.types[t].texture);
//					material.SetTexture("_FrontTex", land.types[t].texture);
//					material.SetTexture("_BackTex", land.types[t].texture);
//				}
//				else if(land.types[t].shaderSize == VoxelBlockType.ShaderSizes.Two){
//					material.SetTexture("_TopTex", land.types[t].topTexture);
//					material.SetTexture("_BottomTex", land.types[t].texture);
//					material.SetTexture("_RightTex", land.types[t].texture);
//					material.SetTexture("_LeftTex", land.types[t].texture);
//					material.SetTexture("_FrontTex", land.types[t].texture);
//					material.SetTexture("_BackTex", land.types[t].texture);
//				}
//				else if(land.types[t].shaderSize == VoxelBlockType.ShaderSizes.Six){
//					material.SetTexture("_TopTex", land.types[t].topTexture);
//					material.SetTexture("_BottomTex", land.types[t].texture);
//					material.SetTexture("_RightTex", land.types[t].rightTexture);
//					material.SetTexture("_LeftTex", land.types[t].leftTexture);
//					material.SetTexture("_FrontTex", land.types[t].frontTexture);
//					material.SetTexture("_BackTex", land.types[t].backTexture);
//				}
			}
			#endregion
		...
		public void ApplyAmbient () 
		{
			...
			{
				VoxelBlock block = visibleFaces[f].block;
		...
		public void  BuildGrass ()
		{
			...
			for (int f=0; f<visibleFaces.Count; f++) 
			{
				...
				if (!visibleFaces[f].block.type.showGrass) continue;
			...
			if (tris.Count != 0)
			{
				...
				grassFilter.GetComponent<Renderer>().castShadows = false;
				...
				for (int h=0; h<typeToSubmesh.Length; h++)
				{
					...
					if (!land.types[h].grassMaterial) continue;
					
					grassMaterials[typeToSubmesh[h]] = land.types[h].grassMaterial;
					matNum++; 
				}
				grassFilter.GetComponent<Renderer>().sharedMaterials = grassMaterials;
			}
			#endregion
		...
		public MeshFilter CreateFilter (string name) //creates new object with filter and mesh
		{
			...
			//if (land==null) land = transform.parent.GetComponent<Voxel>();
			//if (land!=null) obj.renderer.material = mat;
			
			#if UNITY_EDITOR
			//hiding wireframe
			UnityEditor.EditorUtility.SetSelectedWireframeHidden(objFilter.GetComponent<Renderer>(), true);
