...
public class VoxelOctNode
{
	...
	public VoxelOctNode[] nodes;
	
	//TestStruct temp;
	
	
	public VoxelOctNode GetNode (int x, int y, int z, int halfsize) //returns the node at given coordinates
	{
	...
	public VoxelOctNode SetNode (int x, int y , int z, int halfsize) //returns the deepest node at given coords
	{
	...
	*/
	...
	public void  GenerateFromHeightmap (float[,] map1,  float[,] map2,  float[,] map3,  float[,] map4, 
		...
		else
		{
			...
			for (int i=0;i<8;i++) nodes[i] = new VoxelOctNode();
	...
	public void  GenerateFromOctree ( VoxelData data ,   int offsetX ,   int offsetY ,   int offsetZ ,   int halfsize , 
	                                 int dataOffsetX ,   int dataOffsetY ,   int dataOffsetZ  ){
		//finding reference type at this coords
		VoxelOctNode refNode;
		int refType = 0; bool  refExists = false;
	...
	public void  GenerateFromTwoOctrees ( VoxelData data ,   VoxelData backgroundData ,   int offsetX ,   int offsetY ,   int offsetZ ,   int halfsize , 
	                                     int dataOffsetX ,   int dataOffsetY ,   int dataOffsetZ  ){
		//finding reference type at this coords
		VoxelOctNode refNode;
		int refType = 0; bool  refExists = false;
...
public class ChunkData
{
	...
	public VoxelOctNode GetNode (int x, int y, int z)  //returns the node at given coordinates
	{
	...
	{ 
		VoxelOctNode topNode = nodes[y/size];
		VoxelOctNode node = topNode.SetNode(x, y-(y/size)*size, z, size/2);
...
public class VoxelData : ScriptableObject
{
	//int[ matrix,,];
	//float[ ambientMatrix,,];
	public enum VoxelDataGenerateBlend {additive=0, replace=1}; 
	
	static public VoxelOctNode[] nullArray = new VoxelOctNode[0];
	
	[HideInInspector] public  int[] blocks;
	[HideInInspector] public float[] ambient;
	[HideInInspector] public bool[] exists;
	
	[HideInInspector] public VoxelOctNode[] octree;
	...
	Material tempMat;
/*	
	private System.Collections.Generic.List<VoxelUndoNodes> undoList = new System.Collections.Generic.List<VoxelUndoNodes>();
	
	public class VoxelUndoNodes
	{
		public System.Collections.Generic.List<int> nums = new System.Collections.Generic.List<int>();
		public System.Collections.Generic.List<VoxelOctNode> nodes = new System.Collections.Generic.List<VoxelOctNode>();
	}
	
	public void  RegisterUndo ()
	{
		undoList.Add( new VoxelUndoNodes() );
		if (undoList.Count > 10) undoList.RemoveAt(0);
	}
	
	public void  AddUndoNode ( int num  )
	{
		VoxelUndoNodes nodesList= undoList[undoList.Count-1];
	...
	public void  Upgrade (){
		if (blocks.Length==0 || exists.Length==0) { Debug.Log("Voxel: No outdated data found. Possibly already upgraded"); return; }
		
		Debug.Log("Voxel: Upgrading terrain data to new octree format. If something went wrong DO NOT save your project and make a backup of your data file.");
...
*/	
	public VoxelOctNode GetBigNode ( int x ,   int y ,   int z  )  //returns the most parent node
	{
	...
	public VoxelOctNode GetNode ( int x ,   int y ,   int z  )  //returns the particular child node
	{
	...
	public VoxelOctNode GetClosestNode ( int x ,   int y ,   int z  )  //returns child node with safe frame
	{
	...
	{ 
		...
		VoxelOctNode topNode = octree[nZ*nodesY*nodesX + nY*nodesX + nX];
		...
		VoxelOctNode node = topNode.SetNode(x-nX*biggestNode, y-nY*biggestNode, z-nZ*biggestNode, biggestNode/2);
...
*/
	...
	static public VoxelData New ( int newX ,   int newY ,   int newZ ,   int newNode ,   int filledLevel  ){
		#if UNITY_EDITOR
...
[HideInInspector] VoxelDataGenerateBlend genNoiseBlend;
